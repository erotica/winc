...
type SandboxManager interface {
	Create(rootfs string, diskLimit int64) (*ImageSpec, error)
	Delete() error
}

//go:generate counterfeiter . Limiter
type Limiter interface {
	SetDiskLimit(volumePath string, size int64) error
}
...
func (s *sandboxManager) Create(rootfs string, diskLimit int64) (*ImageSpec, error) {
